...
import { Generator as GeneticsjsGenerator } from '../../generator/utils';
...
export class RscriptCommand {
  ...
  static generate(numberOfInstances: number): string[] {
    ...
    const randomGeneratedSeed = GeneticsjsGenerator.generateInteger(new NumericRange(1, 2147483647));
    for (let index = 0; index < numberOfInstances; index++) {
      const parameters = Generator.randomSimulatorParams();
      const instance = RscriptCommand.build(parameters, { type: SeedType.RANDOM_SEARCH, value: randomGeneratedSeed });
